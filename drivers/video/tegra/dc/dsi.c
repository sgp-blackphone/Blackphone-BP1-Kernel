			...
			struct tegra_dc_dsi_data *dsi)
{
	...
	dsi->dsi_control_val =
			...
			DSI_CONTROL_VID_SOURCE(dc->controller_id) |
...
void tegra_dsi_mipi_calibration_14x(struct tegra_dc_dsi_data *dsi)
{
	...
	val = MIPI_CAL_HSCLKPDOSDSIA(0x3) |
		MIPI_CAL_HSCLKPUOSDSIA(0x3);
	tegra_mipi_cal_write(dsi->mipi_cal, val,
	...
	val = MIPI_CAL_OVERIDEDSIA(0x0) |
		...
		MIPI_CAL_HSPDOSDSIA(0x1) |
		MIPI_CAL_HSPUOSDSIA(0x2) |
		MIPI_CAL_TERMOSDSIA(0x6);
	tegra_mipi_cal_write(dsi->mipi_cal, val,
