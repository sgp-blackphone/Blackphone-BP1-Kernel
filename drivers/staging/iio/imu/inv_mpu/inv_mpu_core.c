...
static int nvi_vreg_dis(unsigned int i)
{
	int err = 0;

	if (vreg[i].ret && (vreg[i].consumer != NULL)) {
		err = regulator_disable(vreg[i].consumer);
		if (!err) {
			vreg[i].ret = 0;
			printk("%s %s\n",
				__func__, vreg[i].supply);
		} else {
			printk("%s %s ERR\n",
				__func__, vreg[i].supply);
		}
...
static int nvi_vreg_dis_all()
{
	unsigned int i;
	int err = 0;

	for (i = ARRAY_SIZE(nvi_vregs); i > 0; i--)
		err |= nvi_vreg_dis((i - 1));
	return err;
}

static int nvi_vreg_en(unsigned int i)
{
	int err = 0;

	if ((!vreg[i].ret) && (vreg[i].consumer != NULL)) {
		err = regulator_enable(vreg[i].consumer);
		if (!err) {
			vreg[i].ret = 1;
			printk("%s %s\n",
				__func__, vreg[i].supply);
...
static void nvi_vreg_exit()
{
	int i;

	for (i = 0; i < ARRAY_SIZE(nvi_vregs); i++) {
		if (vreg[i].consumer != NULL) {
			devm_regulator_put(vreg[i].consumer);
			vreg[i].consumer = NULL;
			printk("%s %s\n",
				__func__, vreg[i].supply);
		}
	}
...
static int nvi_vreg_init(struct inv_mpu_state *st)
{
	unsigned int i;
	int err = 0;

	for (i = 0; i < ARRAY_SIZE(nvi_vregs); i++) {
		vreg[i].supply = nvi_vregs[i];
		vreg[i].ret = 0;
		vreg[i].consumer = devm_regulator_get(&st->client->dev,
							  vreg[i].supply);
		if (IS_ERR(vreg[i].consumer)) {
			err = PTR_ERR(vreg[i].consumer);
			dev_err(&st->client->dev, "%s ERR %d for %s\n",
				__func__, err, vreg[i].supply);
			vreg[i].consumer = NULL;
		} else {
			dev_dbg(&st->client->dev, "%s %s\n",
				__func__, vreg[i].supply);
		}
	}
	return err;
		...
		const struct i2c_device_id *id)
{
	...
	if (conf->has_compass) {
		...
		printk("inv_mpu_setup_compass_slave OK!!\n");
	}
