...
static int __devinit tegra_i2c_probe(struct platform_device *pdev)
{
	...
	i2c_dev->needs_cl_dvfs_clock = plat->needs_cl_dvfs_clock;
	...
	i2c_dev->bus_clk_rate = plat->bus_clk_rate ? plat->bus_clk_rate: 100000;
	...
	i2c_dev->bit_banging_xfer_after_shutdown =
			plat->bit_banging_xfer_after_shutdown;
	init_completion(&i2c_dev->msg_complete);
	...
	if (plat->retries)
		i2c_dev->adapter.retries = plat->retries;
	else
		i2c_dev->adapter.retries = TEGRA_I2C_RETRIES;

	if (plat->timeout)
		i2c_dev->adapter.timeout = plat->timeout;
